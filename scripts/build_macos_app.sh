...
uv run python "$SCRIPT_DIR/fetch_ripgrep.py" \
    --manifest "$SCRIPT_DIR/third_party/ripgrep" \
    --destination "$HELPER_DIST"
mkdir -p "$HELPER_DIST/licenses/pcre2"
cp \
    "$SCRIPT_DIR/third_party/pcre2-LICENCE.md" \
    "$HELPER_DIST/licenses/pcre2/LICENCE.md"
if [ "$SIGN_IDENTITY" = "-" ]; then
    codesign --force --sign - "$HELPER_DIST/bin/rg"
else
    codesign \
        --force \
        --options runtime \
...
codesign --verify --strict "$HELPER_DIST/bin/rg"
