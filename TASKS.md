...
> 状态：活文档。只记录当前状态、仍有效的决策、未完成 DoD 和已完成阶段摘要。
> 逐次试跑日志和已结束 milestone 的完整讨论由 Git 历史保留。

## Current Status

更新于 2026-07-20。新会话先读本节，再看 `git log -n 10` 和未完成勾选框。
...
### 当前阶段：M19 Research Idea Composer
...
目标：从用户本地论文库选择一个强 baseline 和三个可兼容模块，输出有证据、
可验证、明确区分事实与假设的中文实验方案。

- [x] 本地 `ccf_a` / `ccf_b` / `other` library 扫描和候选检索
- [x] baseline 只从 CCF A 选择，并记录性能证据与 improvement opening
- [x] module pool 按 CCF A → CCF B → other 顺序 fallback
- [x] 正常成功路径要求 3 个 modules，且来自 3 个不同 paper
- [x] deterministic plan/state 和 final report contract
- [x] proposal checker 与 remediation 边界
- [x] chunk/field evidence ref 可在前端反查
- [x] VI-ReID 严格单例 clean rerun
- [ ] 至少 3 个固定研究任务有人工验收记录（按用户决策暂跳过）
- [ ] 严格预扣或预估下一次 LLM 调用，避免预算尾部超限
- [ ] 对真实跨任务结果统计 evidence coverage / unsupported claim rate

恢复 M19 验收时，优先验证跨任务稳定性，不要继续为同一个 ReID seed 调 prompt。
...
- [x] `uv run mypy`
- [x] `uv run pytest`
- [x] schema validation 一次 retry，并记录失败输入与错误位置
- [ ] GitHub Actions 自动跑 ruff / mypy / pytest（按用户决策暂跳过）
- [ ] smoke eval 在公开步骤下可复现，不依赖未说明的本地 PDF
- [ ] 至少 5 篇论文有 evidence coverage / unsupported claim rate 人工抽样
...
| M16 | Partial | 门禁与 schema retry 完成；CI、公开可复现 eval、人工质量抽样未做 |
...
| M19 | Active | Composer plan、checker 和单例验收完成；跨任务验收未做 |
