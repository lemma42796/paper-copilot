...
| 命令研究顺序 | 私有 `runs/codex-cli-jsonl-v2/` 的逐轮 JSONL 与原生 session rollout；摘要见 `tool_system_v2_plan.md` 2.3 | `rg --files` → `pdfinfo` → `pdftotext -f/-l -layout/-raw` → `rg`/`sed`/`awk` | Runtime preflight 批量准备预算内 cache index，模型直接用 `rg`/`awk` 定位，页级原文通过窄化 broker 读取；不复制 benchmark 答案 |
