...
标注会话读取 14 篇 PDF 和本计划中的 8 条 query，创建结构化 gold。它不运行被测系统，
...
> 请只依据当前目录中的14篇硕士学位论文进行整理，不要使用网络资料。先按“主要研究问题”对论文分类。每篇论文只能确定一个主要类别，但可以补充次要主题。请列出论文题目、作者、主要类别、监督设定和所涉及的模态，并为判断提供原文页码。不要仅根据文件名分类。

#### Query 2

> 从这些论文中找出所有以无监督学习为主要设定的行人重识别研究。比较它们使用的监督信息、核心机制、训练数据组织方式、实验数据集和评价指标。请区分“完全无监督”“无监督域适应”和仅在局部使用伪标签，不要因为论文出现“无监督”一词就将其纳入。

#### Query 3

> 从现在开始，后续讨论排除所有以无监督学习为主要设定的论文。在剩余论文中，找出明确研究以下问题的论文：模态缺失、不完整多模态、行人遮挡。请分别解释这三个概念，说明每篇论文实际解决了哪个问题，并给出方法名称、作者和证据页码。不要把一般的数据增强自动视为遮挡研究。

#### Query 4

> 请回到论文原文，逐项复核上一轮的结论。对每项结论给出论文题目、作者、PDF页码、章节和一段能够直接支持结论的简短原文。如果证据只能部分支持，或者论文没有明确说明，请明确标为“部分支持”或“未能确认”，不要根据常识补全。

#### Query 5

> 继续保持之前排除无监督论文的条件。请找出剩余论文中涉及红外—可见光行人重识别的研究，比较它们的特征对齐或融合位置、训练目标、实验数据集、评价指标和主要结果。数值结果必须说明来自哪篇论文及具体页码；无法统一比较的结果不要强行排序。

#### Query 6

> 在此前排除条件的基础上，再排除以遮挡行人重识别为主要研究问题的论文。对剩余论文比较注意力机制的具体作用：它作用于什么输入或特征、位于网络什么阶段、解决什么问题、如何参与训练。没有使用注意力机制的论文不要为了填满表格而纳入。

#### Query 7

> 判断项莘泽的《基于多模态信息融合的行人轨迹追踪方法研究》在什么情况下应该纳入多模态行人重识别综述，在什么情况下应该排除。请分别检查其行人重识别、轨迹补全和可视化系统部分，不要只给“相关”或“不相关”的结论，并提供章节和页码证据。

#### Query 8

> 请生成最终比较表，并继续遵守此前已经提出的所有排除条件。表格包含：论文、作者、研究问题、监督设定、输入模态、核心方法、注意力或融合位置、训练目标、数据集、评价指标、局限性、证据页码。项莘泽论文只纳入与行人重识别直接相关的内容。每个关键单元格都必须能追溯到具体论文；没有可靠证据时填写“未确认”。表格后列出你在复核过程中修正过的结论。

### 6.3 轨迹保存

用户侧保存完整任务轨迹，不额外告诉被测 Codex。轨迹至少包括：
...
工具实现和产品配置冻结后，开启全新会话，从 Query 1 开始运行相同的 8 轮。
...
> 在本组 14 篇硕士学位论文和固定 8 轮对话中，采用某项工具改造的 Paper Copilot
