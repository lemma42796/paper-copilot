...
| `trash` | 需要批准 | 移入 macOS 系统废纸篓 |
| `restore` | 需要批准 | 恢复旧版本应用回收区的历史回执 |
...
工具不提供永久删除。`trash` 通过 Finder 将 PDF 移入 macOS 系统废纸篓，用户需要恢复时使用 Finder 的“放回原处”。`restore` 仅为旧版本隐藏回收区保留，新的 `trash` 不再生成应用回执。
...
3. 提供 `list / inspect / mkdir / copy / move / trash` 以及兼容历史回执的 `restore` 原语；
4. 将路径限制在用户选择的论文库内，拒绝绝对路径、目录逃逸和覆盖；
5. 对所有写操作持久化审批请求并暂停 job，通过前端、HTTP 或 WebSocket接收用户决定；
6. 用 macOS 系统废纸篓代替永久删除，并为越界、系统废纸篓、审批和输出截断补最小测试。
...
### 为什么新的 `trash` 不提供应用内 `restore`？

Finder 可能为废纸篓中的同名文件改名，而且不同卷的废纸篓位置不同。应用无法可靠绑定原文件与废纸篓条目，因此新文件的恢复交给 macOS 的“放回原处”，避免错误恢复或覆盖。已有 `restore` 只服务于旧版本生成的内部回收回执。
