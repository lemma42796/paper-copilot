...
当前状态同步自 `TASKS.md`，更新时间为 2026-07-20。

Paper Copilot 已经推进到 chat-first 本地研究助手：

- 终端命令行界面已移除；本地 HTTP API 由宿主进程调用
  `paper_copilot.api.http.serve_http_api()` 提供，核心入口是 `POST /chat`。
- `apps/web/` 已有 Next.js macOS 风格 chat shell，可展示资料库、历史报告、运行状态、成本、Composer 摘要和 evidence。
...
Web UI 是当前产品主入口。它依赖宿主进程提供本地 HTTP API；仓库不再暴露
终端启动命令。前端开发仍在 `apps/web/` 中进行：
