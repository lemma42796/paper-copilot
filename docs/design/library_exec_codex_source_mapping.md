...
| 特殊命令拦截 | `unified_exec/exec_command.rs` 中的 `intercept_apply_patch` | 命令解析后、通用进程启动前，把一个窄化命令交给专用 handler | 直接采用 + Codex 缺失 | `paper-cache` 采用相同拦截位置和失败语义；具体缓存操作是论文领域专用能力 |
