...
### `apps/web/`
产品前端采用 Next.js。终端命令行界面已移除；普通用户通过 Web UI 使用产品，
Next.js 前端通过本地 Python API 的持久 job 接口提交任务；`POST /chat` 只保留为
同步兼容入口。
视觉方向采用 ChatGPT 桌面端信息架构：安静的原生桌面壳层、最近会话、中央消息流、
底部输入框，以及可独立完全隐藏的会话栏和研究上下文栏；报告继续使用高可读
Markdown。第一版只做单输入框 chat shell、运行状态、
多轮消息、session/report/eval 路径和 cost/paper budget 展示;不做营销页、复杂
dashboard、登录或云同步。

右侧研究上下文栏包含本地 rollout 诊断面板。面板通过只读 diagnostics API 读取当前
job 的最新 attempt；运行时低频刷新，终态到达后再刷新一次，展示耗时摘要、首错、慢
调用、未完成实体和重复工具签名。它不直接读取 trace 文件，也不承担自动熔断。

### `chat/`
Chat-first 产品入口层。接收裸自然语言请求，组装 knowledge stores / agent
context，调用 `Paper Copilot` 的公开 run 入口，并返回前端可直接消费的
...
一个 conversation 可以包含多个顺序 job；前端按该 id 聚合消息，当前追问创建新 job，
...
task，因此 WebSocket/HTTP interrupt 和 deadline 都作用于同一个 child，但终态不同：用户
...
`session.jsonl`、`job.json` 双写，保持恢复和前端协议不变：session 仍是模型
...
ExtractPaperTool 将来可通过 query 拿到 top-k chunk + 页码。

**刻意与 `knowledge/` 分开**：单篇检索的 query 来自读论文工具的深读意图
...
ExtractPaperTool 全文投喂成本 ¥0.05-0.11/篇，未触及 ¥0.30 预算，retrieval
未证明必要。
...
这里推迟的是结构化字段数组的独立全文索引；M18 已给 chunk text 增加 FTS5，
两者不是同一查询面。`fields_store.upsert` 只存 raw JSON 不
...
不记录 PDF 路径 → 批量重建时按 sha1 paper_id 从 PDF 目录重新匹配,
...
**M18 当前状态(2026-05-21)**：`embeddings.db` 同时维护 chunk FTS5 和
sqlite-vec，`hybrid_search` 用 RRF 融合两路排名，按论文聚合后从每篇候选池
选择 3-5 个非重复 evidence chunks。34 篇 / 36 条 seed query 的 paper
recall@5=98.4%、recall@10=100.0%；13 条 evidence-labeled query 的
evidence recall@5=87.2%、recall@10=89.7%。详细 gate 与指标口径见
`TASKS.md`。
...
- `suite.py` — Pydantic Suite/PaperSpec/BudgetPerPaper schema,YAML
  入。每个 paper 跑 ReadPaperTool.run 前先校 `compute_paper_id(pdf) ==
  paper_id`(防 PDF 换路径),再用 tmpdir 当 PAPER_COPILOT_HOME 起隔离
  run(LinkRelatedPapersTool 自动短路,用户的真索引不动,suite 可重复)。
- `assertions.py` — 纯函数,JSON dict in / `FieldFailure` list out。
- `suite.py` / `history.py` / `report.py` — 运行 suite、记录 run 历史并生成报告；
  由测试或内部调用方直接使用，不暴露终端命令。
...
- 候选 qwen3.6-plus(snapshot 2026-04-02),百炼定价 2.0/2.5/0.2/12.0
  ...
  (prefix match,fail-loud on unknown);`agents/read_paper_tool.py`
...
- `jsonschema.py`：JSON Schema 工具。当前提供 `inline_refs`，把 Pydantic
  ...
  存在理由：qwen3.6-flash 在历史 Anthropic 兼容端点评测中对 `$ref` 处理不可靠（会
...
```
apps/web ──► api ──► chat ──► agents  ◄────────── schemas
                              │
                   ┌──────────┼──────────┐
                   ▼          ▼          ▼
...
- `eval/` 可以调用 `agents/` 的**公开 run 入口**（这样 eval 强制 dogfood
  ...
  `retrieval/`——这样 extractor 内部重构时 eval 不被拖累
...
- **2026-04-27 M15 Session B 复核**:把 `DEFAULT_MODEL` 临时切到 qwen3.6-plus
  跑一次 smoke eval suite,实测 plus **2.03x cost / 2.22x latency / 0
...
前端把当前 job id 写入本地存储，重开页面或 API 重连后重新查询状态和增量事件。
运行期间优先连接独立端口的双向 WebSocket；`job/interrupt` 与 `job/resume` 控制请求
和 `job/events` 通知走同一连接，连接未建立时控制请求回退到 HTTP。事件连接失败后
切换到 HTTP SSE，SSE 也不可用时才使用增量轮询。三者共享 job event `seq` 游标。
queued/running 只恢复显示，不自动
重新执行；interrupted/failed 也保持终态，
直到用户在原会话输入明确的恢复指令，前端才调用 `POST /jobs/<id>/resume`。恢复入口
仍是普通输入框，不增加专用按钮。运行期间发送按钮切换为 ChatGPT 桌面端式的停止按钮，
调用 interrupt API；停止完成后输入框恢复正常发送状态。

已完成任务下继续输入普通问题时，前端沿用当前 `conversation_id` 创建新 job。后端将
...
GET /jobs/<id>/stream?after=N  # SSE 回退流
WS  /jobs/<id>/stream?after=N  # 首选双向事件/控制通道（默认端口 8766）
...
### 为什么把读论文拆成有界工具，而不是 one big prompt？

- 50 页论文塞进 one prompt 会让主上下文充满 PDF 原文，压缩/缓存/后续追问
  都变差。
- 工具边界主要是**上下文隔离和结构化契约手段**，不是多个 Agent 之间的协作。
- 每个工具只接收完成任务所需的输入，并返回经过 schema 校验的结果，避免 PDF
  原文和中间 tool output 污染 Paper Copilot 的主上下文。
...
- **查询 pattern 不同**：单篇检索计划由 ExtractPaperTool 用结构化意图驱动
  （"取 methodology section 的数学定义"），跨论文检索是用户或 LinkRelatedPapersTool
  用自然语言查询。
...
- **解决方案是 reindex 流程**：一次性重建。100 篇论文 × 200 chunks ×
  1024-dim embedding，`text-embedding-v4` 走百炼 API,成本和延迟用真实
  reindex trace 记录。
- **`embeddings_meta.json` 记录当前模型**：防止误用（模型不匹配直接报错）。

### 为什么 compare 工具不走 LLM？

- 如果结构化抽取做得好，对比两篇论文的 Method 字段是 SQL 层的事，不需要
  再调一次模型。
- 这也是对自己的**dogfood 压力**：如果 compare 的输出不够好，说明结构化
  抽取有问题，应该回去改 ExtractPaperTool，而不是用 LLM 掩盖。
...
**M5 已验证**：
- [x] SkimPaperTool 不需要 retrieval，只靠 PDF 前几页 + 目录就够
...
**M5 推迟到后续 milestone 再验**（原 M5 那组里不属于 SkimPaperTool 单点能验的）：
...
- [x] Prompt cache 命中率能到 60%+ — **M9 打脸**:qwen-flash +
      ...
      qwen 版本升级 / 切到其他兼容层 / ExtractPaperTool 改 per-field fan-out
      让单调用 user 块缩到 5K 以内。回归脚本:`scripts/m9_cache_ab.py`
- [x] ExtractPaperTool 每字段一次调用 vs 单次调用输出多字段——哪种准确率更高 — M7
...
- [x] qwen3.6-flash 对 Pydantic 嵌套 schema 的 `$defs` + `$ref` 处理不可靠：
      ...
      `shared/jsonschema.inline_refs` 作为 workaround，SkimPaperTool 三篇嵌套
      字段全部为真 dict。未来触发重新评估的条件:
      (a) qwen 模型版本升级（qwen3.7 / qwen4）
      (b) 切换到 Anthropic 原生或其他兼容层
      (c) M7 ExtractPaperTool 的嵌套 schema 更深（>2 层）时，先不假设 inline_refs
...
- [ ] arxiv API metadata 能否可靠补齐 PaperMeta 的 canonical 字段
      ...
      两处均**非 SkimPaperTool 抽取错误**——模型忠实于 PDF 原样 + 按 schema
...
- [x] qwen3.6-flash 在 forced `tool_choice={"type":"tool",...}` 下不返回
      ...
      entry 在 SkimPaperTool 场景下常态缺位；M7 ExtractPaperTool 若同样用 forced tool，
...
- [x] Dashscope 百炼 Anthropic-compat 对 `cache_control: ephemeral` 的
      **user-message 块存在未公开的大小阈值**（实测约 ~5K tokens)。
      ...
      (c) ExtractPaperTool 改 per-field fan-out 让单调用 user 块 < 5K
      重验方法:`scripts/m9_cache_ab.py on|off` 两两对比 cache_read/create。

**M11 之后审视（跨论文相关）**：
- [x] text-embedding-v4 在当前 seed corpus 的 paper-level 召回率够用
      — 34 篇 / 36 条 query 实测 recall@5=98.4%、recall@10=100.0%。
      13 条 evidence-labeled query 的 evidence recall@5=87.2%，仍作为
      grounding 风险保留；新真实 miss 出现前不继续调同一 seed ranking。
- [~] 50-100 篇规模下，sqlite-vec 的跨库检索 < 500ms
      — 13 篇下 warm 287-973ms;50-100 篇规模无实测,sqlite-vec 线性扫描
      复杂度下预估仍远低于 1s,M14 eval 时正式验。
- [x] LinkRelatedPapersTool 的自动关联粒度：整篇 vs 章节 vs 方法级？
      ...
      要求下不划算;真要章节级先靠 `compare_papers` 工具 dogfood。M14 golden
...
- [x] fields.db 的 SQL schema：结构化字段能否用 JSON column + 表达式索引
      ...
      (a) 库 > 1k 篇时结构化数组 LIKE 是否需独立索引（chunk FTS5 已上线）
...
- [x] `read` 流程新增跨论文步骤后，端到端延迟能否控制在 90s 内
      ...
      **M12 加 LinkRelatedPapersTool 后(2026-04-25)**:Bahdanau --force 重读
      LinkRelatedPapersTool 段 latency 2.1s(query encode + KNN + 1 次 LLM 调用),
