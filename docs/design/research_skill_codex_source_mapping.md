...
| Skill version | Codex metadata 没有满足本计划评测要求的独立版本字段 | Skill 可由 path/name 识别，但不能单独冻结内容版本 | Codex 缺失：使用显式版本 `1` 加完整正文 SHA-256；评测可同时冻结人读版本和精确内容 |
...
- `inspect_page` 和 `paper_set` 尚未实施。Skill 在需要视觉关系时标记 `unresolved`，在
  全量覆盖未完成时标记 `incomplete`，不伪造未来工具。
